...
const contains = (s: string, ...subs: string[]) =>
    subs.some((x) => s.includes(x));
...
export function iconForMime(mimeRaw?: string): JSX.Element {
    ...
    if (contains(mime, "pgp-signature", "x-pem-file", "pkcs", "x-x509")) return <FileKey className={iconClass} />;
...
export function labelForMime(mimeRaw?: string): string {
    ...
    if (
        mime.startsWith("text/") ||
        contains(mime, "markdown", "rtf", "xml")
    )
        return "Documents";
